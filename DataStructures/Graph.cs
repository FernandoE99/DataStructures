...
namespace DataStructures
{
    public interface Graph <T, K>
    {
        ...
        void AddVertices(IEnumerable<T> vertices); //Adds to the graph the vertices contained in a set, if it exist already it is skipped
        bool DeleteVertex(T vertex); //Deletes a vertex from the graph

        void DeleteVertices(IEnumerable<T> vertexSet); //Deletes every vertex of the set existing also in the graph
